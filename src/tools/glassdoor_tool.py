...
class GlassdoorTool:
    ...
    async def _get_client(self):
        ...
        if not HAS_HTTPX:
            raise ImportError("httpx is required for GlassdoorTool. Install with: pip install httpx")
    ...
    async def get_company_data(self, company_name: str) -> Optional[GlassdoorCompanyData]:
        ...
        try:
            ...
            response = await client.get(
                f"https://{self.rapidapi_host}/company/{quote_plus(company_name)}",
                headers=headers,
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Glassdoor API error: {response.status_code}")
                return await self._fallback_company_search(company_name)
    ...
    async def get_recent_reviews(
        ...
        try:
            ...
            response = await client.get(
                f"https://{self.rapidapi_host}/reviews/{quote_plus(company_name)}",
                headers=headers,
                params={"limit": limit},
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Glassdoor reviews API error: {response.status_code}")
                return []
    ...
    async def get_salary_data(
        ...
        try:
            ...
            response = await client.get(
                f"https://{self.rapidapi_host}/salaries/{quote_plus(company_name)}",
                headers=headers,
                params=params,
            )
            ...
            if response.status_code == 200:
                ...
                return {"error": f"API error: {response.status_code}", "salaries": []}
