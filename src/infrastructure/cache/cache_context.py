...
from src.services.data import get_cache_service, CacheService
            ...
            try:
                ...
                if cached is not None:
                    ...
                    if isinstance(cached, dict) and "_cache_meta" in cached:
                        ...
                        self._cached_at = cached.get("_cache_meta", {}).get("cached_at")
                    else:
                        self._result = cached
...
async def cache_context(
    ...
    Example:
        async with cache_context("my_key") as cache:
            ...
            cache.result = result
    """
    ctx = CacheContext(key=key, mode=mode, ttl_seconds=ttl_seconds, namespace=namespace)
    async with ctx:
        yield ctx
