...
class CrossCompanyURLCache:
    ...
    def get(
        ...
        with self._lock:
            ...
            logger.debug(
                f"Cross-company cache hit: {url} " f"(used by: {entry.companies_used})"
            )
    ...
    def get_potential_shares(self, company: str) -> List[str]:
        ...
        with self._lock:
            return [
                url
                for url, entry in self._cache.items()
                if company not in entry.companies_used and self._is_shareable(url)
            ]
