...
class EntityExtractionResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "companies": [
                {"name": c.name, "relationship": c.relationship} for c in self.companies
            ],
            "financials": [
                {"metric": f.metric, "value": f.value, "period": f.period}
...
- Include context for financial figures (what period, what scope)"""
                ...
                if f.get("metric") and f.get("value"):
                    result.financials.append(
                        Financial(
                            metric=f["metric"],
                            value=f["value"],
                            normalized_value=self._parse_number(
    ...
    def get_financial_metrics(
        ...
        if metric_type:
            return [
                f for f in merged.financials if metric_type.lower() in f.metric.lower()
            ]
