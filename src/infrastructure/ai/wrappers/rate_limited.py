...
from src.infrastructure.resilience.rate_limiting import get_rate_limiter_manager, RateLimitConfig
...
class RateLimitedAIClient(BaseAIClient):
    ...
    async def generate(
        ...
        if not acquired:
            from src.infrastructure.exceptions import AIRateLimitError
