...
if TYPE_CHECKING:
    from ..services.persistent_source_registry import PersistentSourceRegistry
...
class RelevanceScore:
    ...
    high_value_domain_bonus: float = 0.0  # Bonus for domains that provided useful data before
...
class RelevanceFilter:
    ...
    GAP_KEYWORDS: Dict[str, List[str]] = {
        "Revenue": ["revenue", "ingresos", "sales", "ventas", "financial", "earnings", "facturación"],
        "Market Share": ["market share", "cuota de mercado", "participación", "share %", "market position"],
        "Subscribers": ["subscribers", "suscriptores", "usuarios", "customers", "abonados", "clientes"],
        ...
        "Founded": ["founded", "fundada", "established", "since", "year founded", "history"],
    ...
    def __init__(
        ...
        official_str = f", official={self._official_domain}" if self._official_domain else ""
        logger.info(
    ...
    def calculate_score(self, result: Dict, target_query_gaps: Optional[List[str]] = None) -> RelevanceScore:
        """
        ...
        if self.company in snippet:
            ...
            if matches:
                score.company_in_snippet = (
                    self.WEIGHTS["company_in_snippet"] *
                    (len(matches) / len(self.company_words))
                )
        ...
        if any(ind in url for ind in self.SPANISH_INDICATORS):
            ...
            score.language_match = self.WEIGHTS["language_match"] * 0.7  # Slightly less for English
    ...
    def _extract_official_domain(self, official_website: Optional[str]) -> Optional[str]:
    ...
    def _is_official_url(self, url: str) -> bool:
        ...
        try:
            ...
            return (
                domain == self._official_domain or
                domain.endswith("." + self._official_domain)
            )
        except Exception:
            return False
...
def create_incremental_filter(
    ...
    """
    from ..services.persistent_source_registry import get_source_registry
    from ..services.existing_data_analyzer import get_data_analyzer
