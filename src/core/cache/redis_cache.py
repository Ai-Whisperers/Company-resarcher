...
Usage:
    from src.core.cache.redis_cache import get_redis_cache, RedisDeadLetterQueue
...
class RedisCacheConfig:
    ...
    password: Optional[str] = field(default_factory=lambda: _get_redis_settings().password)
    ...
    max_connections: int = field(default_factory=lambda: _get_redis_settings().max_connections)
...
class RedisCache:
    ...
    def _build_url(self) -> str:
        ...
        return f"{protocol}://{auth}{self.config.host}:{self.config.port}/{self.config.db}"
...
class RedisCircuitBreaker:
    ...
    async def record_failure(self):
        ...
        if state == CircuitState.HALF_OPEN:
            ...
            logger.warning(f"Circuit breaker for {self.service_name} OPEN (half-open failure)")
            return
