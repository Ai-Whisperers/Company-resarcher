...
class PriorityQuery(BaseModel):
    ...
    target_sources: List[str] = Field(
        ...
        description="Specific domains or source types to target"
    )
...
class EntityIdentifier(BaseModel):
    ...
    identifier_type: str = Field(..., description="Type: headquarters, founding_year, industry, etc.")
...
class AlternativeSource(BaseModel):
    ...
    primary_sources: List[str] = Field(default_factory=list, description="Primary sources to try")
    fallback_strategy: str = Field(default="", description="What to do if primary fails")
...
class CompanyHypothesis(BaseModel):
    ...
    business_description: str = Field(
        default="",
        description="What the company does based on name/URL/industry"
    )
    company_type: str = Field(
        default="private",
        description="public/private/government/subsidiary/nonprofit"
    )
    ownership_hypothesis: str = Field(
        default="",
        description="Likely parent company or ownership structure"
    )
    key_products_services: List[str] = Field(
        default_factory=list,
        description="Likely main products or services"
    )
    target_markets: List[str] = Field(
        default_factory=list,
        description="Likely target customer segments"
    )
    ...
    industry_position: str = Field(
        default="",
        description="Likely market position (leader/challenger/niche/etc.)"
    )
...
class ResearchIntelligenceBrief(BaseModel):
    ...
    irrelevance_keywords: List[str] = Field(
        default_factory=list,
        description="Keywords that indicate irrelevant content"
    )
    recency_importance: str = Field(
        default="medium",
        description="How important is data recency: high/medium/low"
    )
    min_date_preference: Optional[str] = Field(
        default=None,
        description="Prefer sources after this date (YYYY-MM-DD)"
    )

    # === Gap Awareness ===
    predicted_gaps: List[ResearchGap] = Field(
        default_factory=list,
        description="Information that will likely be hard to find"
    )
    alternative_sources: List[AlternativeSource] = Field(
        default_factory=list,
        description="Fallback strategies for missing information"
    )
    ...
    confidence_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="AI's confidence in this brief (0-1)"
    )
    reasoning: str = Field(
        default="",
        description="AI's reasoning for the hypotheses and priorities"
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ResearchIntelligenceBrief":
        ...
        if "company_hypothesis" in data and isinstance(data["company_hypothesis"], dict):
        ...
        if "priority_queries" in data:
            for key, queries in data["priority_queries"].items():
                data["priority_queries"][key] = [
                    PriorityQuery(**q) if isinstance(q, dict) else q
                    for q in queries
                ]
    ...
    def calculate_relevance_score(self, title: str, content: str) -> float:
        ...
        relevance_count = sum(
            1 for kw in self.relevance_keywords
            if kw.lower() in text
        )

        # Count irrelevance keywords
        irrelevance_count = sum(
            1 for kw in self.irrelevance_keywords
            if kw.lower() in text
        )
