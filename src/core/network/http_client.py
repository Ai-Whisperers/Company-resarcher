...
Usage:
    from src.core.network.http_client import get_http_client, http_get, http_post
...
from ..resilience.rate_limiting import get_rate_limiter_manager
...
class HTTPClient:
    ...
    def __new__(cls, config: Optional[HTTPClientConfig] = None):
        ...
        **kwargs,
        ...
        for attempt in range(self._config.max_retries + 1):
            try:
                ...
                logger.debug(
                    f"HTTP {method} {url} -> {response.status} ({duration:.2f}s)"
                )
                ...
                if response.status in self._config.retry_on_status:
                    if attempt < self._config.max_retries:
                        ...
                        logger.warning(
                            ...
                            f"retrying in {retry_after:.1f}s (attempt {attempt + 1})"
                        )
                ...
                if attempt < self._config.max_retries:
                    ...
                    logger.error(f"HTTP request failed after {attempt + 1} attempts: {e}")
                    raise
