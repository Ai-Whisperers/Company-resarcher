...
class ContradictionAnalysisResult:
    ...
    def get_unresolved(self) -> List[Contradiction]:
        ...
        return [c for c in self.contradictions_found
    ...
    def get_resolved_values(self) -> Dict[str, str]:
        ...
        return {
            ...
            if c.resolved_value and c.resolution_confidence != ResolutionConfidence.UNRESOLVED
        }
...
class AIContradictionResolver:
    ...
    async def analyze_claims(
        ...
        result = ContradictionAnalysisResult(
            ...
            high_severity_count=sum(1 for c in contradictions
                                   if c.severity == ContradictionSeverity.HIGH),
            resolved_count=sum(1 for c in contradictions
                              if c.resolution_confidence != ResolutionConfidence.UNRESOLVED),
            unresolved_count=sum(1 for c in contradictions
                                if c.resolution_confidence == ResolutionConfidence.UNRESOLVED),
        )
