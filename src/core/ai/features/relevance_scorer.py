...
class RelevanceScore:
    ...
    reason: str
    predicted_content_type: str = ""  # e.g., "financial_data", "news", "company_profile"
...
class AIRelevanceScorer:
    ...
    async def score_results(
        ...
        # Log filtering stats
        above_threshold = [r for r in scored_results if r.relevance.score >= self.threshold]
        logger.debug(
...
IMPORTANT: Return scores for ALL {len(results)} results."""
    ...
    def filter_by_threshold(
        ...
        return [
            ...
            if r.relevance.score >= threshold
        ]
