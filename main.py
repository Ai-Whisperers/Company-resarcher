            ...
            if company.has_financial_data_available():
                try:
                    ...
                    if financial_service.is_available():
                        print(f"  Fetching financial data for {company.get_effective_ticker()}...")
                        financial_result = await financial_service.fetch_financial_data(company)
                        if financial_result and financial_result.has_data:
                            # Generate financial reports to temp path, then read them
                            import tempfile
                            with tempfile.TemporaryDirectory() as tmpdir:
                                ...
                                for report_name, report_path in reports.items():
                                    ...
                                    drafts[f"06-Data-Room/financial_data/{report_path.name}"] = content
                            print(f"  Financial data: {len(reports)} reports generated")
                            if financial_result.is_parent_data:
                                print(f"    (Data from parent company: {financial_result.parent_name})")
                        else:
                            print(f"  Financial data: No data available for {company.get_effective_ticker()}")
                    else:
                        logger.debug("Alpha Vantage API key not configured, skipping financial data")
